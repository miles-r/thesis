...
for (folder in regions) {
    ...
    for (csv in files) {
        ...
        else region_df %<>% full_join(df, by = "date") 
       
    }
    
    #region_df %<>% select(Date, TimeStep, everything())
    region_df %<>% mutate(year = lubridate::year(date) %>% as.integer,
    ...
    region_df %<>% arrange(year, month)
    
    lags = region_df  %>%
        transmute_if(is.double, lag)
    lags %<>% select(id, everything()) %>%
        mutate(id = id + 1)
    region_df %<>% full_join(lags, by = c("id"), suffix = c("", "_lag1")) 
...
coagmet = bind_rows(EC, NE, NWMA, SE, SLV, SW) %>% 
    select(year, month, location, precip_avg, precip_min, precip_max, everything()) %>% 
    arrange(year)
