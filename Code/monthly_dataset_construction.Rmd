...
dmh %<>% group_by(ams_hay_region) %>% make_lags( vars(D0, D1, D2, D3, D4), 4)
dmc %<>% group_by(ams_corn_region) %>% make_lags( vars(D0, D1, D2, D3, D4), 4)
```

```{r join}
hay %<>% full_join(dmh, by = c("location" = "ams_hay_region", "year", "month"))
corn %<>% full_join(dmc, by = c("location" = "ams_corn_region", "year", "month"))
```

```{r write}
write_csv(hay, paste0(fakepath, "thesis/Data/AMS/hay_co_monthly_dm.csv"))
write_csv(corn, paste0(fakepath, "thesis/Data/AMS/corn_co_monthly_dm.csv"))
```
